...
// Thanks Daniela Bento (OLX) for help in Teams processing
func ListOfTeamsDetectorProc(detector *detector.Detector) []cty.Value {
	var teamsValue []cty.Value

	if detector.Teams == nil {
		return teamsValue
	}

	for i := range detector.Teams {
		teamsValue = append(teamsValue, cty.StringVal(detector.Teams[i]))
	}
	return teamsValue
}
